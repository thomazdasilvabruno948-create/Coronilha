...
export default function App() {
  ...
  return (
    <main>
      ...
      <section id="inicio" className="hero"><div className="hero-copy"><p className="eyebrow">ENCONTRE SEU PRÓXIMO ENDEREÇO</p><h1>Espaços para viver<br /><em>boas histórias.</em></h1><p>Imóveis selecionados para quem valoriza localização, qualidade e tranquilidade.</p><a className="button" href="#catalogo">Explorar imóveis</a></div><div className="hero-image"><img src={destaque.imagem} alt={destaque.titulo} /><div className="hero-card"><small>EM DESTAQUE</small><strong>{destaque.titulo}</strong><span>{destaque.localizacao} · {destaque.preco}</span></div><div className="arrows"><button onClick={() => setIndice((indice + imoveis.length - 1) % imoveis.length)}>←</button><button onClick={() => setIndice((indice + 1) % imoveis.length)}>→</button></div></div></section>
      <section className="categories"><button onClick={() => setCategoria("Imóveis")}>Imóveis <span>↗</span></button><button onClick={() => setCategoria("Terrenos")}>Terrenos <span>↗</span></button><button onClick={() => setCategoria("Todos")}>Loteamentos <span>↗</span></button></section>
